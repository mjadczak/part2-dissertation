...
An algorithm to ensure this, originating in Beam \cref{BEAM-code-WatermarkManager}, is summarised in the appendix (\cref{apx:additional:watermark-holds}).
...
The overall behaviour of the FSM is best showcased by looking at the \exs{TriggerDriver} behaviour\footnote{An Elixir \emph{behaviour} is a contract for a module to implement. An implementing module can then be chosen or provided at runtime and its functions (with signatures conforming to the behaviour) called dynamically by another module.} from the Elixir implementation in \cref{lst:impl:trigger_driver}.
...
The trigger driver can be asked if it is finished using the \exs{finished?} callback.
...
The FSM pattern provides great flexibility in specifying triggering behaviour, but keeps the interface to this logic very simple.
...
The \exs{namespace} of a timer is an arbitrary value, but in practice is used to associate timers to the Triggers that set them, or to keep track of different timer purposes in specialised Transforms.
...
Sources for popular data sources such as the file system, Apache Kafka, Google BigQuery and others are included in the Beam implementation, and an API \cite{BEAM-code-SourceAPI} is provided to add custom ones with arbitrary watermark logic (in fact, this API is used in \cref{sec:eval:code}).
...
While it would be possible to have the \exs{use Dataflow} statement redefine the \exs{--} operator as one used for labelling Transforms, it would likely cause confusion.
...
Elixir macros are powerful tools, allowing arbitrary AST transformation at compile-time \cite[p.\ todo]{Elixir-Metaprogramming}.
...
This is a convention \cite[p.\ todo]{Elixir-Metaprogramming} which ensures that the macros do not end up causing unintuitive behaviour for the user.
...
The \exs{options <> transform} syntax is also transformed, but is currently used to pass additional options for testing purposes only.

The macro outputs a new AST node which calls the \exs{Dataflow.__apply_transform__} function which contains the Transform application logic.
...
The \exs{use Dataflow} line in the examples imports the \exs{~>} macro implicitly (as well as setting up some module aliases).
In fact, a \exs{use Module} statement just calls the \exs{Module.__using__} macro \cite[p.\ todo]{Elixir-Metaprogramming}.
...
Branching Pipelines such as the one in \cref{fig:todo} can be created by applying multiple Transforms to one Collection.
...
\todo{Explain how NestedState is used to track Transform hierarchy?}
...
\todo{OTP, DirectRunner, GenServers, Supervisors, GenStage, TimingManager/ReducingEvaluator}
